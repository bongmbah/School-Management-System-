       ...
       <div>
            <?php
               ...
               //get courses lecturer teaches for HND1 semester 1
                $sem1_course_query = "SELECT departmentcourses.C_NAME FROM departmentcourses INNER JOIN semester1_schedule   
                                      ON(departmentcourses.C_CODE = semester1_schedule.C_CODE) WHERE semester1_schedule.LECT_CODE = '$lect_code' UNION
                                      SELECT generalcourses.C_NAME FROM generalcourses INNER JOIN semester1_schedule   
                                      ON(generalcourses.C_CODE = semester1_schedule.C_CODE) WHERE semester1_schedule.LECT_CODE  = '$lect_code'";
                $sem1_course_result = $connect->query($sem1_course_query);
                $sem1_courses = array();        
                 $i = 0;
                while($sem1_course_rows = $sem1_course_result->fetch_assoc()){
                     $sem1_courses[$i] = $sem1_course_rows['C_NAME'];
                     $i =$i + 1;
                }

                //get courses lecturer teaches for HND1 semester 2
                $sem2_course_query =  "SELECT departmentcourses.C_NAME FROM departmentcourses INNER JOIN semester2_schedule   
                                       ON(departmentcourses.C_CODE = semester2_schedule.C_CODE) WHERE semester2_schedule.LECT_CODE = '$lect_code' UNION
                                       SELECT generalcourses.C_NAME FROM generalcourses INNER JOIN semester2_schedule   
                                       ON(generalcourses.C_CODE = semester2_schedule.C_CODE) WHERE semester2_schedule.LECT_CODE  = '$lect_code'";
                $sem2_course_result = $connect->query($sem2_course_query);
                $sem2_courses = array();        
                 $i = 0;
                while($sem2_course_rows = $sem2_course_result->fetch_assoc()){
                     $sem2_courses[$i] = $sem2_course_rows['C_NAME'];
                     $i =$i + 1;
                }

                //get courses lecturer teaches for HND2 semester 3
                $sem3_course_query =  "SELECT departmentcourses.C_NAME FROM departmentcourses INNER JOIN semester3_schedule   
                                       ON(departmentcourses.C_CODE = semester3_schedule.C_CODE) WHERE semester3_schedule.LECT_CODE = '$lect_code' UNION
                                       SELECT generalcourses.C_NAME FROM generalcourses INNER JOIN semester3_schedule   
                                       ON(generalcourses.C_CODE = semester3_schedule.C_CODE) WHERE semester3_schedule.LECT_CODE  = '$lect_code'";
                $sem3_course_result = $connect->query($sem3_course_query);
                $sem3_courses = array();        
                 $i = 0;
                while($sem3_course_rows = $sem3_course_result->fetch_assoc()){
                     $sem3_courses[$i] = $sem3_course_rows['C_NAME'];
                     $i =$i + 1;
                }
        ...
        <form action="" class="heading" method="POST">
            ...
            <div class="table-credentials">
                ...
                <div>
                    ...
                    ?>
                      <option><?php echo $c_level;?></option>
                    <?php 
                        $m++;
                       }
                    ?>
            ...
            <table>
                <tr class="table-h">
                    ...
                    <th class="remark">Remark</th>
                     ...
                     <?php
                        // values could not be gotten through after button click so i used session to the values
                        if($_POST['fillscore']){
                            $level = $_POST['level'];
                            $course = $_POST['course'];
        ...
        <script>
            ...
            var sem1courses = <?php echo json_encode($sem1_courses); ?>;
            var sem2courses = <?php echo json_encode($sem2_courses); ?>;
            var sem3courses = <?php echo json_encode($sem3_courses); ?>;
            var sem4courses = <?php echo json_encode($sem4_courses) ;?>;

            level.addEventListener('change',()=>{
                if(level.value == "HND 1 semester 1"){
                    course.innerHTML = ``;
                    for(i=0;i< sem1courses.length;i++){
                        course.innerHTML +=`<option>${sem1courses[i]}</option>`;
                    }

                }
                else if(level.value == "HND 1 semester 2"){
                    course.innerHTML = ``;
                    for(i=0;i< sem2courses.length;i++){
                        course.innerHTML +=`<option>${sem2courses[i]}</option>`;
                    }
                }
                else if(level.value == "HND 2 semester 1"){
                    course.innerHTML = ``;
                    for(i=0;i< sem3courses.length;i++){
                        course.innerHTML +=`<option>${sem3courses[i]}</option>`;
                    }
                }
                else if(level.value == "HND 2 semester 2"){
                    course.innerHTML = ``;
                    for(i=0;i< sem4courses.length;i++){
                        course.innerHTML +=`<option>${sem4courses[i]}</option>`;
                    }
                }else{
                    course.innerHTML = ``;
                }
